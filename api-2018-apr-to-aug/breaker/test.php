...
//function clean($wambua_corrupted_employees) {
  // $wambua_corrupted_employees = str_replace(' ', '-', $wambua_corrupted_employees); // Replacing all spaces with hyphens.

//  $wambua_corrupted_employees = preg_replace('/[^+&()%0-9\-]/', '', $wambua_corrupted_employees);
  //return preg_replace('/[^+&()%0-9\-]/', '', $wambua_corrupted_employees); // Removes special characters.
//}
//echo $wambua_corrupted_employees;
$text = explode(",",$wambua_corrupted_employees);
?>
<?php echo (preg_replace('/[()0-9>+=\/%?^-]/','',ucwords(strtolower($wambua_corrupted_employees))));
?>
